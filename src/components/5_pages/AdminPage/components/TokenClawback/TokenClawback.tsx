import { Button, Field, Flex, Heading, Input, Stack, Text } from "@chakra-ui/react";
...
const TokenClawback = () => {
  ...
  useEffect(() => {
    if (isClawbackPending || isClawbackConfirming) {
      setIsClawingBack(true);
    } else if (!isClawbackPending && !isClawbackConfirming && !isClawbackSuccess) {
      setIsClawingBack(false);
    }
    ...
    >
      <Stack gap="0.5rem">
        ...
        <Text fontSize="sm" color="fg.muted">
          Deactivate all active epochs for a specific token and clawback all unclaimed tokens. This will affect all epochs using the specified token.
        </Text>
      </Stack>
